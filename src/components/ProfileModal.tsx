...
export const ProfileModal: React.FC<ProfileModalProps> = ({ onClose }) => {
  ...
  // Active Sessions & Multi-Device State ("نشست‌های فعال و انداختن بیرون دستگاه")
  ...
  const handleTerminateSession = async (sessionId: string) => {
    ...
    try {
      ...
      setSessions(updated);
      setSessionToast('نشست دستگاه با موفقیت خاتمه یافت و دستگاه مربوطه از حساب کاربری خارج گردید.');
      setTimeout(() => setSessionToast(null), 4000);
      sounds.playComplete();
    } catch (err: any) {
      alert(err.message || 'خطا در خاتمه نشست دستگاه.');
    } finally {
  ...
  const handleTerminateAllOtherSessions = async () => {
    if (!window.confirm('آیا از خاتمه دادن به تمامی نشست‌ها در سایر دستگاه‌ها و انداختن آن‌ها به بیرون اطمینان دارید؟')) {
      return;
    }
    setIsTerminatingAll(true);
    sounds.playTrash();
    try {
      const updated = await api.terminateAllOtherSessions();
      setSessions(updated);
      setSessionToast('کلیه نشست‌های فعال در سایر دستگاه‌ها با موفقیت خاتمه یافتند.');
      setTimeout(() => setSessionToast(null), 4000);
      sounds.playComplete();
    } catch (err: any) {
      alert(err.message || 'خطا در خروج از سایر دستگاه‌ها.');
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Active Sessions & Devices ("نشست‌های فعال و قابلیت انداختن بیرون دستگاه") */}
        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-3.5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="p-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 text-slate-600 transition-colors text-xs cursor-pointer shadow-2xs"
                title="به‌روزرسانی نشست‌ها"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isLoadingSessions ? 'animate-spin text-indigo-600' : ''}`} />
              </button>
              ...
              {sessions.filter(s => !s.isCurrent).length > 0 && (
                <button
                  ...
                  className="px-2.5 py-1 rounded-xl bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 text-[11px] font-bold transition-all flex items-center gap-1 cursor-pointer shadow-2xs"
                  title="خاتمه تمام نشست‌ها به جز دستگاه فعلی"
                >
                  <LogOut className="w-3.5 h-3.5" />
                  <span>{isTerminatingAll ? 'در حال خروج...' : 'خروج از سایر دستگاه‌ها'}</span>
                </button>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2.5">
            {sessions.length === 0 ? (
              <div className="p-3.5 rounded-xl bg-white border border-slate-200/60 flex items-center justify-between gap-3 shadow-2xs">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-bold">این دستگاه (نشست جاری)</span>
                    </div>
                    <div className="text-[10px] text-slate-400 mt-0.5">
                      موقعیت: {city || province || 'ایران'} • آخرین فعالیت: هم‌اکنون
                    </div>
                  </div>
                </div>
                      ...
                      <div className="text-xs font-bold text-slate-800 flex items-center gap-1.5 flex-wrap">
                        ...
                        {s.isCurrent && (
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold border border-emerald-200">
                            این دستگاه (نشست جاری)
                          </span>
                        )}
                      </div>
                      <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-2 flex-wrap">
                        <span>موقعیت: {s.location || 'ایران'}</span>
                        <span>•</span>
                        <span className="font-mono" dir="ltr">{s.ip}</span>
                        <span>•</span>
                        <span>فعالیت: {s.isCurrent ? 'هم‌اکنون' : s.lastActive ? s.lastActive.replace('T', ' ').slice(0, 16) : 'اخیراً'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end">
                    {s.isCurrent ? (
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs hover:shadow-xs"
                        title="خاتمه دادن به این نشست و خروج اجباری دستگاه"
                      >
                        <Trash2 className="w-3.5 h-3.5 text-rose-600" />
                        <span>{terminatingSessionId === s.id ? 'در حال قطع...' : 'انداختن بیرون دستگاه'}</span>
                      </button>
                    )}
                  </div>
                </div>
